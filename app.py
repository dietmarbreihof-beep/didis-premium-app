...
@app.route('/stop-loss-strategien')
def stop_loss_strategien():
    """Stop-Loss wie die Profis - Lance Breitstein Masterclass"""
    track_visitor()
    
    # Prüfe ob es ein entsprechendes Modul in der DB gibt
    module = None
    try:
        module = LearningModule.query.filter_by(slug='stop-loss-strategien').first()
    except:
        pass
    ...
    if not is_admin and user_subscription not in ['premium', 'elite', 'elite_pro', 'masterclass']:
        flash('Für dieses Modul benötigst du ein Premium-Abonnement.', 'warning')
        return redirect(url_for('upgrade_required', module_slug='stop-loss-strategien'))
    ...
    return render_template('stop-loss-strategien.html', 
                         module=module, 
                         prev_module=prev_module, 
                         next_module=next_module)

@app.route('/traders-journey')
def traders_journey():
    """Deine Trader-Reise - Die 4 Stufen zum profitablen Trading"""
    track_visitor()
    
    # Prüfe ob es ein entsprechendes Modul in der DB gibt
    module = None
    try:
        module = LearningModule.query.filter_by(slug='traders-journey').first()
    except:
        pass
    ...
    if not is_admin and user_subscription not in ['premium', 'elite', 'elite_pro', 'masterclass']:
        flash('Für dieses Modul benötigst du ein Premium-Abonnement.', 'warning')
        return redirect(url_for('upgrade_required', module_slug='traders-journey'))
    ...
    return render_template('traders-journey.html', 
                         module=module, 
                         prev_module=prev_module, 
                         next_module=next_module)
...
@app.route('/wie-man-trader-wird-3')
def wie_man_trader_wird_3():
    """Wie man Trader wird Teil 3 - Setups & Expertise aufbauen (StockBee)"""
    track_visitor()
    ...
    if not is_admin and user_subscription not in ['premium', 'elite', 'elite_pro', 'masterclass']:
        flash('Für dieses Modul benötigst du ein Premium-Abonnement.', 'warning')
        return redirect(url_for('upgrade_required', module_slug='wie-man-trader-wird-3'))
    
    return render_template('wie-man-trader-wird-3.html')


@app.route('/moving-averages-qullamaggie')
def moving_averages_qullamaggie():
    """Moving Averages - Qullamaggie's Strategie (Market Wizards Edition)"""
    track_visitor()
    ...
    if not is_admin and user_subscription not in ['premium', 'elite', 'elite_pro', 'masterclass']:
        flash('Für dieses Modul benötigst du ein Premium-Abonnement.', 'warning')
        return redirect(url_for('upgrade_required', module_slug='moving-averages-qullamaggie'))
